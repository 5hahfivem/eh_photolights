...
local function StartPreview(itemId, model, itemName)
    ...
    lib.showTextUI('Press F to place, G to cancel, Scroll Up/Down to rotate', {
...
local function ConfirmPlacement()
    ...
    TriggerServerEvent('eh:photolights:dropProp', itemId, vector4(coords.x, coords.y, coords.z, heading), itemName)
    return true
end
...
local function PickupDrop(data)
    ...
    TriggerServerEvent('eh:photolights:pickupDrop', state.propDrop.id)
end
    ...
    target:addModel(models, {
        {
            ...
            onSelect = function(data)
                ...
                TriggerServerEvent('eh:photolights:setcolour', id, {
                    colour = colour,
                    label = input[1]
                })
            end
